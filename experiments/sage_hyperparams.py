...
def run_ray_tuner(
    ...
    config = {
        ...
        # sampling config
        "num_neighbors": num_neighbors,  # tune.choice([16, 32, 64, 128]),
        # model config
        "channels": channels,  # tune.choice([32, 64, 128]),
        "num_layers": num_layers,  # tune.choice([1, 2, 3, 4, 5]),
        "aggr": aggr,  # tune.choice(["max", "sum"]),
    }
    ...
    tuner = tune.Tuner(
        tune.with_resources(
            ...
            resources={"cpu": 1, "gpu": 0.1 if use_gpu else 0},
        ),
