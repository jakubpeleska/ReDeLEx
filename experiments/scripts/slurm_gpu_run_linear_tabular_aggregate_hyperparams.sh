#!/bin/bash
#SBATCH --job-name=linear_tabular_aggregate_hyperparams  
...
EXPERIMENT_NAME="linear_tabular_aggregate_hyperparams"
...
python -u experiments/tabular_hyperparams.py --ray_address=${ray_address} \
  ...
  --dataset=${dataset} --task=${task} --tabular_model="linear" --aggregate_neighbors \
  --num_samples=${NUM_SAMPLES} --num_gpus=1 --num_cpus=${SLURM_CPUS_PER_GPU} &> "${log_dir}/run.log"
